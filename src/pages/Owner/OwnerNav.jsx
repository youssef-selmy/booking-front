...
const OwnerNav = () => {
  return (
    <header className="w-full h-[70px] border-b border-[#ddd] flex fixed bg-white">
      {pages.map((ele, idx) => (
        <NavLink key={idx} to={ele.url} className={({isActive}) => [
      ...
      <Link to="/owner" className="w-[350px] flex justify-center items-center bg-[#333] text-white"><IoIosLogOut size={25} /></Link>
    </header>
  );
};
